...
class TestValidationFunctions:
    ...
    @pytest.mark.asyncio
    async def test_validate_input_success(self, hass: HomeAssistant, mock_transport_nsw_config_flow):
        """Test successful input validation."""
        mock_transport_nsw_config_flow.get_departures.return_value = {"route": "T1"}
        data = {CONF_API_KEY: "test_api_key", CONF_NAME: "Custom Name"}

        result = await validate_input(hass, data)

        assert result["title"] == "Custom Name"
        mock_transport_nsw_config_flow.get_departures.assert_called_once_with(
            "10101100", "", "", "test_api_key"
        )

    @pytest.mark.asyncio
    async def test_validate_input_success_no_custom_name(self, hass: HomeAssistant, mock_transport_nsw_config_flow):
    ...
    @pytest.mark.asyncio
    async def test_validate_input_short_api_key(self, hass: HomeAssistant, mock_transport_nsw_config_flow):
    ...
    @pytest.mark.asyncio
    async def test_validate_input_api_error(self, hass: HomeAssistant, mock_transport_nsw_config_flow):
        """Test input validation with API error."""
        mock_transport_nsw_config_flow.get_departures.side_effect = Exception("API Error")
    ...
    @pytest.mark.asyncio
    async def test_validate_subentry_input_success(self, hass: HomeAssistant, mock_transport_nsw_config_flow):
    ...
    @pytest.mark.asyncio
    async def test_validate_subentry_input_none_response(self, hass: HomeAssistant, mock_transport_nsw_config_flow):
        """Test subentry validation with None response."""
        mock_transport_nsw_config_flow.get_departures.return_value = None
        data = {CONF_STOP_ID: "123"}
    ...
    @pytest.mark.asyncio
    async def test_validate_subentry_input_api_error(self, hass: HomeAssistant, mock_transport_nsw_config_flow):
...
class TestTransportNSWConfigFlow:
    ...
    @pytest.mark.asyncio
    async def test_form_valid_input(self, hass: HomeAssistant, mock_transport_nsw_config_flow):
        ...
        result2 = await hass.config_entries.flow.async_configure(
            result["flow_id"],
            {CONF_API_KEY: "test_api_key", CONF_NAME: "Test Name"},
        )

        assert result2["type"] is FlowResultType.CREATE_ENTRY
        assert result2["title"] == "Test Name"
        assert result2["data"] == {CONF_API_KEY: "test_api_key", CONF_NAME: "Test Name"}

    @pytest.mark.asyncio
    async def test_form_cannot_connect(self, hass: HomeAssistant, mock_transport_nsw_config_flow):
        """Test form with connection error."""
        mock_transport_nsw_config_flow.get_departures.side_effect = Exception("Connection error")
...
class TestTransportNSWSubentryFlowHandler:
    ...
    async def test_subentry_flow_form_show(self, hass: HomeAssistant):
        ...
        result = await hass.config_entries.subentries.async_init(
            (parent_entry.entry_id, SUBENTRY_TYPE_STOP),
    ...
    @pytest.mark.asyncio
    async def test_subentry_flow_success(self, hass: HomeAssistant, mock_transport_nsw_config_flow):
        ...
        result2 = await hass.config_entries.subentries.async_configure(
            result["flow_id"],
            {CONF_STOP_ID: "123", CONF_ROUTE: "T1", CONF_DESTINATION: "Hornsby"},
        )

        assert result2["type"] is FlowResultType.CREATE_ENTRY
        assert result2["title"] == "Stop 123 (T1 → Hornsby)"
        assert result2["unique_id"] == f"{parent_entry.entry_id}_123_route_T1_dest_Hornsby"

    @pytest.mark.asyncio
    async def test_subentry_flow_cannot_connect(self, hass: HomeAssistant, mock_transport_nsw_config_flow):
    ...
    async def test_subentry_flow_unexpected_exception(self, hass: HomeAssistant):
        ...
        with patch("custom_components.transport_nsw.config_flow.validate_subentry_input", side_effect=RuntimeError("Unexpected")):
            result2 = await hass.config_entries.subentries.async_configure(
                result["flow_id"],
                {CONF_STOP_ID: "123"},
            )
    ...
    async def test_subentry_reconfigure_form(self, hass: HomeAssistant):
        ...
        flow = TransportNSWSubentryFlowHandler()
        flow._set_entry(parent_entry)
        flow._set_reconfigure_subentry(subentry)
    ...
    @pytest.mark.asyncio
    async def test_subentry_reconfigure_success(self, hass: HomeAssistant, mock_transport_nsw_config_flow):
        """Test successful subentry reconfiguration."""
        mock_transport_nsw_config_flow.get_departures.return_value = {"route": "T2"}
        ...
        with patch.object(flow, "async_update_and_abort") as mock_update:
            result = await flow.async_step_reconfigure({
                CONF_STOP_ID: "456",
                CONF_ROUTE: "T2",
    ...
    @pytest.mark.asyncio
    async def test_subentry_reconfigure_cannot_connect(self, hass: HomeAssistant, mock_transport_nsw_config_flow):
